...
public class ConnectionManagerSubscriptionCallback extends BaseSubscriptionCallback {

    private static final String TAG = ConnectionManagerSubscriptionCallback.class.getSimpleName();

    public ConnectionManagerSubscriptionCallback(org.teleal.cling.model.meta.Service service, Context context) {
        super(service, context);
    }

    @Override
    protected void eventReceived(GENASubscription subscription) { // 这里进行 事件接收处理
        Log.i(TAG, "SubscriptionConnectionManager:" + subscription.toString());
